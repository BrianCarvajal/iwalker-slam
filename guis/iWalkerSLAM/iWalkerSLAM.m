function varargout = iWalkerSLAM(varargin)
    % IWALKERSLAM MATLAB code for iWalkerSLAM.fig
    %      IWALKERSLAM, by itself, creates a new IWALKERSLAM or raises the existing
    %      singleton*.
    %
    %      H = IWALKERSLAM returns the handle to a new IWALKERSLAM or the handle to
    %      the existing singleton*.
    %
    %      IWALKERSLAM('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in IWALKERSLAM.M with the given input arguments.
    %
    %      IWALKERSLAM('Property','Value',...) creates a new IWALKERSLAM or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before iWalkerSLAM_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to iWalkerSLAM_OpeningFcn via varargin.
    %
    ...
    gui_State = struct('gui_Name',       mfilename, ...
                       ...
                       'gui_OpeningFcn', @iWalkerSLAM_OpeningFcn, ...
                       'gui_OutputFcn',  @iWalkerSLAM_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before iWalkerSLAM is made visible.
function iWalkerSLAM_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to iWalkerSLAM (see VARARGIN)

    % Choose default command line output for iWalkerSLAM
    ...
    %% Main vertical separation: |leftPanel|mapAxes|tabPanel|
    ...
    handles.tabPanel = uiextras.TabPanel('Parent', hbf);
    
    %% Tab panel
    % Tab 1: iWalker settings 
    tab1 = uiextras.VBox('Parent', handles.tabPanel);
        %Sample times panel
        handles.stPanel = uiextras.Panel('Parent', tab1, 'Title', 'Sample Times');
            g = uiextras.Grid('Parent', handles.stPanel);
                %Labels
                vb = uiextras.VButtonBox('Parent', g);
                    textLabel(vb, 'Odometry');
                    textLabel(vb, 'IMU');
                    textLabel(vb, 'Forces');
                    textLabel(vb, 'Lidar');
                %Inputs
                vb = uiextras.VButtonBox('Parent', g);
                    handles.odometrySampleTimeInput = inputText(vb, @odometrySampleTimeInput_Callback);
                    handles.imuSampleTimeInput = inputText(vb, @imuSampleTimeInput_Callback);
                    handles.forcesSampleTimeInput = inputText(vb, @forcesSampleTimeInput_Callback);
                    handles.laserSampleTimeInput = inputText(vb, @laserSampleTimeInput_Callback);           
            set(g, 'ColumnSizes', [-1 -1]);
            
        %Lidar setup panel
        handles.lidarPanel = uiextras.Panel('Parent', tab1, 'Title', 'Lidar');
            vb = uiextras.VButtonBox('Parent', handles.lidarPanel);
                handles.loadPortsButton = pushButton(vb, 'Load Ports', @loadPortsButton_Callback);
                handles.laserCOMInput = popupMenu(vb, '-', @laserCOMInput_Callback);
 
        %Reactive Control panel
        handles.rcPanel = uiextras.Panel('Parent', tab1, 'Title', 'Reactive Control');
            g = uiextras.Grid('Parent',  handles.rcPanel);
                %Lambdas
                hb = uiextras.HButtonBox('Parent', g);
                    textLabel(hb, 'Lambdas');
                    handles.leftLambdaInput = inputText(hb, @leftLambdaInput_Callback);
                    handles.rightLambdaInput = inputText(hb, @rightLambdaInput_Callback);
                %Nus
                hb = uiextras.HButtonBox('Parent', g);
                    textLabel(hb, 'Nus');
                    handles.leftNuInput = inputText(hb, @leftNuInput_Callback);
                    handles.rightNuInput = inputText(hb, @rightNuInput_Callback);          
            set(g, 'RowSizes', [-1 -1]);
            
        %Map settings panel
        handles.msPanel = uiextras.Panel('Parent', tab1, 'Title', 'Map');
            g = uiextras.Grid('Parent', handles.msPanel);
                %X Limits
                hb = uiextras.HButtonBox('Parent', g);
                    textLabel(hb, 'X Lim');
                    handles.xLimMinInput = inputText(hb, @xLimMinInput_Callback);
                    handles.xLimMaxInput = inputText(hb, @xLimMaxInput_Callback);
                %Y Limits
                hb = uiextras.HButtonBox('Parent', g);
                    textLabel(hb, 'Y Lim');
                    handles.yLimMinInput = inputText(hb, @yLimMinInput_Callback);
                    handles.yLimMaxInput = inputText(hb, @yLimMaxInput_Callback);
                    
                % Resolution
                hb = uiextras.HButtonBox('Parent', g);
                    textLabel(hb, 'Resolution');
                    uiextras.Empty('Parent', hb);
                    handles.resolutionInput = inputText(hb, @resolutionInput_Callback);
                
                % Type of map (8bits or double)
                handles.bitModeInput = checkbox(g, '8 Bits Mode', @bitModeInput_Callback);
                
                % Apply button
                handles.applyMapButton = pushButton(g, 'Apply', @applyMapButton_Callback);
                
            set(g, 'RowSizes', [-1 -1 -1 -1 -1]); 
    
    % Tab 2: SLAM settings
    uiextras.VBoxFlex('Parent', handles.tabPanel);
    
    % Tab 2: Plots settings
    h = uiextras.VBoxFlex('Parent', handles.tabPanel);
        hb = uiextras.HButtonBox('Parent', h);
    % Tab names 
    handles.tabPanel.TabNames = {'iWalker', 'SLAM', 'Plots'};
    handles.tabPanel.SelectedChild = 1;
    
    %% Left panel: /LaserAxes/InfoBox/
    ...
    set( hbf, 'Sizes', [-1 -1.5 -0.5], 'Spacing', 6 );
...
function varargout = iWalkerSLAM_OutputFcn(hObject, eventdata, handles) 
...
function h = inputText(parent, callback)
    h = uicontrol('Parent', parent, ...
                  'Style', 'edit', ...
                  'String', '-', ...
...
function h = pushButton(parent, text, callback)
    h = uicontrol('Parent', parent, ...
                  'Style', 'pushbutton', ...
                  'String', text, ...
                  'callback', callback);
...
%     end
    ...
    if ~isempty(splitAndMergeInfo)
        plMap.plotLineSegments(splitAndMergeInfo.segments, 'segments');
    end
    %% Polar plot
...
function handles = initObjects(handles)
    % Lidar
    handles.lid = RPLIDAR(); 
...
function setState(hObject, state)  
    ...
    switch state
        ...
        case 'READY'
            ...
            set(handles.tabPanel, 'Visible', 'on');
            guidata(hObject, handles);
        case 'RUN'
            set(handles.tabPanel, 'Visible', 'off');
